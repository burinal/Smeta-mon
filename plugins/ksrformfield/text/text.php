...
class plgKsrformfieldText extends JPlugin 
{
	...
	public function onDisplayFieldView($field_params)
	{
		...
		$field_params->params['system-name'] = isset($field_params->params['system-name']) ? $field_params->params['system-name'] : 0;
	...
	{
		JForm::addFormPath(JPATH_ROOT . '/plugins/ksrformfield/text/assets/forms');

		$form = JForm::getInstance('ksrformfield.text.'.$name, $name, array(
			'control' => '',
			'load_data' => true
		));
