...
JHtml::script('mod_km_product_sale/default.js', false, true);
if ($km_params->get('modules_styles', true)) {
	JHtml::stylesheet('mod_km_product_sale/default.css', false, true);
}
KSSystem::loadJSLanguage(false);
JHtml::_('behavior.tooltip');

require_once(dirname(__file__) . DS . 'helper.php');
$modKMProductSaleHelper = new modKMProductSaleHelper();
$products                = $modKMProductSaleHelper->getList($params);
$class_sfx               = htmlspecialchars($params->get('moduleclass_sfx'));

if (count($products) > 0) {
	require JModuleHelper::getLayoutPath('mod_km_product_sale', $params->get('layout', 'default'));
}
