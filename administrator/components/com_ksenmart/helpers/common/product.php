...
class KSMProducts extends KSCoreHelper
{
	...
	public static function getProducts($products = array())
	{
		...
		if (count($products) > 3)
		{
			...
			if (!empty($products))
			{
				$db    = JFactory::getDbo();
				$query = $db->getQuery(true);
				$query->select('c.*')->from('#__ksenmart_products AS c')
                    ->where('c.id IN (' . implode(',', $products) . ')');
				$db->setQuery($query);
					    ...
					    switch ($key)
						{
							case 'files':
								if (isset($eager[$id]))
								{
                                    ...
                                    $product->ordering   = $eager[$id]->ordering;
	...
	private static function prepareProduct(&$product)
	{
		...
		$product->link           = JRoute::_('index.php?option=com_ksenmart&view=product&id=' . $product->id . ':' . $product->alias . '&parent_id=' . $product->parent_id . '&Itemid=' . KSSystem::getShopItemid());
	...
	public static function productPricesTransform(&$product, $convert = true)
	{
		if ($convert)
		{
	...
	public static function getFiles($ids)
	{
		...
		$query = $db->getQuery(true);
		$query
			->select('owner_id, filename, folder, ordering, params')
