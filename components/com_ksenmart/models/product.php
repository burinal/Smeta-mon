...
class KsenMartModelProduct extends JModelKSForm
{
	...
	private function getProductRelations($pid)
	{
		...
		if (!empty($pid) && $pid > 0)
		{
            ...
            foreach($sets as $elem){
                $query = $this->_db->getQuery(true);
                $query
                    ->select('m.*')
                    ->from('#__ksenmart_files as m')
                    ->where('m.owner_id = ' . $elem->id)
                    ->andwhere('m.ordering = 2');
                $this->_db->setQuery($query);
                $back_image = $this->_db->loadObject();
                if (!empty($back_image))
                {
                    $elem->back_img = KSMedia::resizeImage($back_image->filename, $back_image->folder, $this->_params->get('thumb_width'), $this->_params->get('thumb_height'));
                }
            }
	...
	public function getProductComments($pid)
	{
		...
		if (!empty($pid) && $pid > 0)
		{
			...
			$query->select('
                ...
                ->leftJoin('#__ksen_users AS kmu ON kmu.id=c.user_id')
                ->leftJoin('#__users AS u ON kmu.id=u.id')
                ->leftJoin('#__ksenmart_files AS uf ON uf.owner_id=u.id')
                ->where('c.product_id=' . $this->_db->escape($pid))->where('c.published=1')->where('c.parent_id=0')->order('c.date_add DESC');
